...
with tf.Session() as sess:
    ...
    out_op1 = sess.run([op1, filter1])
    print(out_op1[0].shape, out_op1[1].shape)
    print("op1:\n", out_op1)  # 1-1  后面补0
    print("------------------")

    out_op2 = sess.run([op2, filter2])
    print(out_op2[0].shape, out_op2[1].shape)
    print("op2:\n",out_op2 )  # 1-2多卷积核 按列取
    print("op3:\n", sess.run([op3, filter3]))  # 1-3
    print("------------------")

    out2_2 = sess.run([op4, filter4])
    print("op4:\n", out2_2)  # 2-2    通道叠加
