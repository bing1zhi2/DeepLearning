...
def shuffle(input_image, ratio):
    ...
    channels = int(int(shape[2]) / ratio / ratio)
    shuffled = np.zeros((height, width, channels), dtype=np.uint8)
    for i in range(0, height):
        for j in range(0, width):
            for k in range(0, channels):
                shuffled[i, j, k] = input_image[
                    int(i / ratio), int(j / ratio), int(k * ratio * ratio + (i % ratio) * ratio + (j % ratio))]
    return shuffled


def prepare_images(params):
    ratio, training_num, lr_stride, lr_size = params['ratio'], params['training_num'], params['lr_stride'], params[
        'lr_size']
        ...
        for filename in filenames:
            ...
            for x in range(0, horizontal_number):
                for y in range(0, vertical_number):
                    hr_sub_image = hr_image[y * hr_stride: y * hr_stride + hr_size,
                                   x * hr_stride: x * hr_stride + hr_size]
                    lr_sub_image = lr_image[y * lr_stride: y * lr_stride + lr_size,
                                   x * lr_stride: x * lr_stride + lr_size]
...
def prepare_data(params):
    ...
    image_dirs = [d.format(ratio) for d in
                  [params['training_image_dir'], params['validation_image_dir'], params['test_image_dir']]]
...
if __name__ == '__main__':
    ...
    print("Preparing images with scaling ratio: {}", params['ratio'])
