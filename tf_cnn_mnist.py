...
mnist = input_data.read_data_sets('G:\ML\dataset\MNIST_data_bak', one_hot=True)
...
h_fc_plus = tf.nn.relu(tf.matmul(h_fc1, W_fc_plus) + b_fc_plus)
...
cross_entryopy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y_conv),1))

train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entryopy)
